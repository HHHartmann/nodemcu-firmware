...
void prepare_shift(lua_State* L, pixbuf * buffer, int shiftValue, unsigned shift_type, int pos_start, int pos_end){
 
  state->shift.type   = shift_type;
  state->shift.offset = pos_start;
  state->shift.window = pos_end - pos_start;
  state->shift.shift  = shiftValue;
}
...
static int ws2812_effects_set_mode(lua_State* L) {
  ...
  switch (state->effect_type) {
    ...
    case WS2812_EFFECT_RAINBOW_CYCLE:
      ...
      prepare_shift(L, state->buffer, 1, PIXBUF_SHIFT_CIRCULAR, 1, -1);
      break;
    case WS2812_EFFECT_FLICKER:
    ...
    case WS2812_EFFECT_CYCLE:
      ...
      prepare_shift(L, state->buffer, state->effect_int_param1, PIXBUF_SHIFT_CIRCULAR, 1, -1);
      break;
    case WS2812_EFFECT_COLOR_WIPE:
